...
bool TextClass::SetCPU(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, cpuString, 20, 50, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetMousePosition(int NumOfPolygons, int mouseY, ID3D11DeviceContext* deviceContext)
{
	char mouseString[64];
	bool result;
	char tempString[64];
	NumOfPolygons += 1452 * 10;
	_itoa_s(NumOfPolygons, tempString, 10);
	strcpy_s(mouseString, "Num Of Objects = 28, Num Of Polygons = ");
	...
	strcpy_s(mouseString, "Resolution : 1920 x 1080 ");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, mouseString, 20, 110, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetTitle(ID3D11DeviceContext* deviceContext)
{
	...
	strcpy_s(mouseString, "Move : W, A, S, D");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence1, mouseString, 850, 450, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Setup the mouseY string.
	strcpy_s(mouseString, "Rotation : Mouse");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, mouseString, 850, 480, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	strcpy_s(mouseString, "B993113 RYU");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, mouseString, 850, 510, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	strcpy_s(mouseString, "Start !  Press ' 1 ' Key");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, mouseString, 850, 700, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
